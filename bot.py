...
def main():
    ...
    # ── Web App sunucusunu arka planda başlat ──
    import threading, os
    try:
        from webapp_server import run_server
        # Railway PORT'u kullan (hem bot hem webapp aynı portta çalışır)
        _port = int(os.getenv("PORT", os.getenv("WEBAPP_PORT", 8080)))
        _host = os.getenv("WEBAPP_HOST", "0.0.0.0")
        _t = threading.Thread(target=run_server, args=(_host, _port), daemon=True)
        _t.start()
        logger.info(f"🌐 Web App sunucu başlatıldı port:{_port}")
    except Exception as _e:
        logger.warning(f"⚠️ Web App sunucu başlatılamadı: {_e}")
