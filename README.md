...
## 🚂 Railway'de Kurulum (Önerilen)

### 1. GitHub'a yükle
```bash
git init
git add .
git commit -m "parti bot v4"
git remote add origin https://github.com/KULLANICI/parti-bot.git
git push -u origin main
```

### 2. Railway'de proje oluştur
1. [railway.app](https://railway.app) → **New Project** → **Deploy from GitHub repo**
2. Repo'nu seç → otomatik build başlar
...
| `WEBAPP_URL` | `https://PROJE-ADIN.up.railway.app/mac` |
| `ADMIN_IDS` | Telegram user ID'n (örn: `123456789`) |

> `WEBAPP_URL`'i Railway deploy bittikten sonra **Settings → Domains** kısmından kopyala

### 4. Volume ekle (DB kalıcı olsun)
Railway dashboard → **+ New** → **Volume**
- Mount path: `/data`
- Bu sayede `parti.db` her deploy'da sıfırlanmaz

### 5. Bitti!
Railway otomatik olarak hem botu hem web app'i aynı process'te çalıştırır.
...
## 🎮 Web App — Animasyonlu 2D Maç İzleyici

Maç bittikten sonra **"🎮 Maçı Animasyonlu İzle"** butonuna basınca Telegram içinden açılan bir Mini App:

- ⚽ **Gerçek zamanlı 2D saha animasyonu** (Canvas)
- 🔵🔴 Her takımın oyuncuları (formasyon pozisyonlarında) hareket eder
- 🥅 Goller animasyonlu gelir, skor patlar
- 🟡🔴 Kart/sakatlanma olayları ticker'da görünür
- ⏱ 90 dakika (gerçek hızda veya 2x/4x hızlandırılmış)
- 🔄 Maçı istediğin kadar tekrar izleyebilirsin

### Kurulum

**1. Flask bağımlılığını kur:**
```bash
pip install flask
```
...
Telegram Web App'ler **HTTPS** gerektiriyor. İki seçenek:

**A) ngrok ile (geliştirme için):**
```bash
ngrok http 8080
# Çıkan URL'i kopyala, örn: https://abc123.ngrok.io
export WEBAPP_URL="https://abc123.ngrok.io/mac"
python bot.py
```

**B) VPS / Sunucu (production için):**
```bash
# nginx ile reverse proxy kur, SSL sertifikası al
export WEBAPP_URL="https://senin-domain.com/mac"
python bot.py
```

Bot başlarken Web App sunucusu **otomatik olarak** 8080 portunda başlatılır.
