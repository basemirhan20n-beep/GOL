...
# Web App URL — ngrok / VPS adresinle değiştir, ya da .env'den oku
WEBAPP_URL = os.getenv("WEBAPP_URL", "http://localhost:8080/mac")
...
async def cmd_satin_al(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Satın almadan önce satıcı bilgisini al (fiyat kaydedilmeden önce)
    oyuncu_oncesi = fdb.oyuncu_getir(oyuncu_id)
    sonuc = fdb.satin_al(user.id, takim["takim_id"], oyuncu_id)
    # futbol_db artık (basari, mesaj, satici_uid) döndürüyor
    if len(sonuc) == 3:
        basari, mesaj, satici_uid = sonuc
    else:
        basari, mesaj = sonuc
        satici_uid = None
    para = fdb.para_getir(user.id)
    await update.message.reply_text(f"{mesaj}\n💰 Kalan: *{para:,}₺*", parse_mode="Markdown")
    # ── Transfer DM bildirimi: satıcıya özelden haber ver ──
    if basari and satici_uid and satici_uid != user.id and oyuncu_oncesi:
        try:
            await context.bot.send_message(
                chat_id=satici_uid,
                text=(
                    f"💸 *Transfer Haberi!*\n"
                    f"*{oyuncu_oncesi['isim']}* ({oyuncu_oncesi['pozisyon']}, Güç:{oyuncu_oncesi['guc']}) "
                    f"*{takim['isim']}* tarafından satın alındı!\n"
                    f"💰 Hesabına *{oyuncu_oncesi['satis_fiyati']:,}₺* yatırıldı. ✅"
                ),
                parse_mode="Markdown"
            )
        except Exception:
...
async def cmd_mac(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lig_adi = LIG1_ADI if lig == 1 else LIG2_ADI
    # ── Kadro & Form durumu ──
    ev_kadro  = fdb.takim_oyunculari(mac["ev_takim_id"])
    dep_kadro = fdb.takim_oyunculari(mac["dep_takim_id"])
    ev_fit    = sum(1 for o in ev_kadro if not o.get("sakat") and not o.get("kart_cezali"))
    dep_fit   = sum(1 for o in dep_kadro if not o.get("sakat") and not o.get("kart_cezali"))

    # ── Güç barı (5 blok) ──
    def _guc_bari(guc, max_guc=100):
        dolu = round((guc / max_guc) * 5)
        return "█" * dolu + "░" * (5 - dolu)

    ev_bar  = _guc_bari(ev_guc)
    dep_bar = _guc_bari(dep_guc)

    # ── Son maçlar (W/D/L) ──
    def _form_str(takim_id):
        maclar = fdb.son_maclar_takim(takim_id, limit=5) if hasattr(fdb, "son_maclar_takim") else []
        simgeler = []
        for m in maclar:
            if m.get("galibiyet"):
                simgeler.append("✅")
            elif m.get("beraberlik"):
                simgeler.append("🟡")
            else:
                simgeler.append("❌")
        return " ".join(simgeler) if simgeler else "—"

    ev_form  = _form_str(mac["ev_takim_id"])
    dep_form = _form_str(mac["dep_takim_id"])

    metin = (
        f"╔══════════════════════════╗\n"
        f"║  ⚽ *{lig_adi}*\n"
        f"║  📅 *{mac['hafta']}. Hafta Maçı*\n"
        f"╠══════════════════════════╣\n"
        f"║ 🏠 *{ev_t['isim']}*\n"
        f"║    Güç: `{ev_bar}` {ev_guc}\n"
        f"║    🎯 {ev_t.get('taktik','4-4-2')}  👥 {ev_fit}/{len(ev_kadro)} fit\n"
        f"║    Form: {ev_form}\n"
        f"╠═══════════ VS ═══════════╣\n"
        f"║ ✈️ *{dep_t['isim']}*\n"
        f"║    Güç: `{dep_bar}` {dep_guc}\n"
        f"║    🎯 {dep_t.get('taktik','4-4-2')}  👥 {dep_fit}/{len(dep_kadro)} fit\n"
        f"║    Form: {dep_form}\n"
        f"╚══════════════════════════╝\n"
        f"\n🎮 _Maçı başlatmak için aşağıdaki butona bas!_"
    )
    markup = InlineKeyboardMarkup([
        [
        ...
        [
            InlineKeyboardButton(f"💰 {ev_t['isim'][:10]}'e Bahis", callback_data=f"bahis_{mac['mac_id']}_{mac['ev_takim_id']}"),
            InlineKeyboardButton(f"💰 {dep_t['isim'][:10]}'e Bahis", callback_data=f"bahis_{mac['mac_id']}_{mac['dep_takim_id']}"),
        ],
        [
            InlineKeyboardButton("📊 Lig Tablosu", callback_data=f"lig_tab_{lig}"),
            InlineKeyboardButton("👥 Takımım", callback_data="takim_bilgi"),
        ],
        [
            InlineKeyboardButton("❌ Kapat", callback_data="futbol_iptal"),
        ],
    ])
    await _gonder(update, metin, markup=markup)
...
async def futbol_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    # Piyasa
    if data == "piyasa_goster" or data.startswith("piyasa_"):
        await query.answer()
        if data == "piyasa_noop":
            return True
        sayfa = 0 if data == "piyasa_goster" else int(data.split("_")[1])
        await _piyasa_goster(update, sayfa)
        return True
    ...
    if data.startswith("mac_bas_"):
        ...
        # ── Web App URL parametrelerini hazırla ──
        def _enc(s): return urllib.parse.quote(str(s), safe="")
        ev_list = "|".join(sonuc.get("olaylar", []))
        es      = "|".join(sonuc.get("ev_gol_atanlar",  []))
        ds      = "|".join(sonuc.get("dep_gol_atanlar", []))
        webapp_url = (
            f"{WEBAPP_URL}"
            f"?ev={_enc(sonuc['ev_takim'])}"
            f"&dep={_enc(sonuc['dep_takim'])}"
            f"&eg={sonuc['ev_gol']}&dg={sonuc['dep_gol']}"
            f"&et={_enc(sonuc.get('ev_taktik','4-4-2'))}"
            f"&dt={_enc(sonuc.get('dep_taktik','4-4-2'))}"
            f"&w={sonuc['hafta']}"
            f"&ev_list={_enc(ev_list)}"
            f"&es={_enc(es)}&ds={_enc(ds)}"
        )

        markup = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("🎮 Maçı Animasyonlu İzle", web_app=WebAppInfo(url=webapp_url)),
            ],
            [
                InlineKeyboardButton("⚽ Sonraki Maç", callback_data="mac_oyna"),
                InlineKeyboardButton("📊 Lig Tablosu", callback_data=f"lig_tab_{sonuc.get('lig',1)}"),
            ],
            [
                InlineKeyboardButton("👥 Takımım", callback_data="takim_bilgi"),
                InlineKeyboardButton("🛒 Transfer Pazarı", callback_data="piyasa_goster"),
            ],
        ])
